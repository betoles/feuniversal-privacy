...
import { TRADITIONS, getTradition } from './data/traditions.js?v=10.8.6';
import { INTENTIONS, CANONICAL_INTENTIONS, EMOTIONAL_STATES, getCanonicalIntention } from './data/intentions.js?v=10.8.6';
import { PRAYERS_DB } from './data/prayers-db.js?v=10.8.6';
import { PrayerCorpusService } from './services/prayer-corpus-service.js?v=10.8.6';
import { StorageService } from './services/storage-service.js?v=10.8.6';
import { soundManager } from './services/sound-service.js?v=10.8.6';
import { renderIcon, renderLangBadge } from './components/icons.js?v=10.8.6';
import { t, isRTL } from './data/i18n.js?v=10.8.6';
import { STOPWORDS_BY_LANG, matchesTraditionInclusive, getSharedTraditions, getEcumenicalBadgeText } from './data/cross-traditions.js?v=10.8.6';
import { prayerMatchesEmotionCanonical } from './data/emotion-taxonomy.js?v=10.8.6';
import { NotificationService } from './services/notification-service.js?v=10.8.6';

import { OnboardingComponent } from './components/onboarding.js?v=10.8.6';
import { MirrorReaderComponent } from './components/mirror-reader.js?v=10.8.6';
import { AltarComponent } from './components/altar.js?v=10.8.6';
import { BeadCounterComponent } from './components/bead-counter.js?v=10.8.6';
import { VaultComponent } from './components/vault.js?v=10.8.6';
import { SpiritualCompassComponent } from './components/spiritual-compass.js?v=10.8.6';
import { SacredHabitComponent } from './components/sacred-habit.js?v=10.8.6';
import { SacredSoundPicker } from './components/sacred-sound-picker.js?v=10.8.6';
import { NovenaModalComponent } from './components/novena-modal.js?v=10.8.6';
import { NotificationModalComponent } from './components/notification-modal.js?v=10.8.6';
import { ScripturesViewComponent } from './components/scriptures-view.js?v=10.8.6';
import { MembershipComponent } from './components/membership.js?v=10.8.6';
import { LanguageModalComponent } from './components/language-modal.js?v=10.8.6';
import { SacredDialog } from './components/sacred-dialog.js?v=10.8.6';
import { PrivacyModalComponent } from './components/privacy-modal.js?v=10.8.6';
...
export class FeUniversalApp {
  ...
  async renderDashboard() {
    ...
    // 2. Gráfico Donut de Oraciones y Compendio Sagrado Universal (4,312 oraciones)
    const totalUniversalCompendium = 4312;
  ...
  getFilteredPrayers(availablePrayers, lang) {
    ...
    if (hasSearch) {
      ...
      for (const p of fullSourceList) {
        ...
        if (titleLang.includes(rawQuery) || titleOrig.includes(rawQuery)) {
          score += 250;
        } else if (pId.includes(rawQuery.replace(/\s+/g, '_'))) {
          score += 200;
        } else if (tradText.includes(rawQuery)) {
          score += 100;
        }
        ...
        for (const token of queryTokens) {
          ...
          const isWordMatchTitle = new RegExp(`(^|[\\s,.:;\\-_(/'"])${token}([\\s,.:;\\-_)/'"]|$)`).test(titleLang);
          const isWordMatchId = new RegExp(`(^|_)${token}(_|$)`).test(pId);

          if (isWordMatchTitle) {
            score += 140;
            tokenMatched = true;
          } else if (titleLang.includes(token) || titleOrig.includes(token)) {
            score += 80;
            tokenMatched = true;
          }
